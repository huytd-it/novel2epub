...
function TranslateTab({ slug, server, meta }: { slug: string; server: EbookSettings["translate"]; meta: EbookSettings["meta"] }) {
  ...
  const fields: FieldSpec<EbookSettings["translate"]>[] = [
    ...
    { key: "ai_glossary_analysis", label: "AI tự phân tích glossary từng chương", kind: "checkbox", hint: "Sau mỗi chương, LLM đề xuất tên riêng/thuật ngữ mới vào hàng chờ duyệt; không tự ghi đè glossary." },
    {
      key: "context_note",
    ...
    {
      key: "auto_cleanup_han",
      label: "Tự động dọn chữ Hán còn sót",
      kind: "checkbox",
      hint: "Chạy sau khi dịch xong từng chương. Chỉ sửa phần còn chữ Hán và giữ bản gốc để đối chiếu.",
    },
    {
      key: "cleanup_han_engine",
      label: "Engine dọn chữ Hán",
      kind: "select",
      options: [
        { value: "openai", label: "LLM / AI biên tập" },
        { value: "local_mt", label: "Local MT (offline)" },
      ],
      disabledWhen: (v) => !v.auto_cleanup_han,
    },
    {
      key: "cleanup_han_max_chars",
      label: "Giới hạn ký tự mỗi lượt dọn Hán",
      kind: "number",
      hint: "Chỉ áp dụng cho LLM; 0 = không giới hạn.",
      disabledWhen: (v) => !v.auto_cleanup_han || v.cleanup_han_engine !== "openai",
    },
    {
      key: "cleanup_han_retries",
      label: "Số lần thử lại dọn Hán",
      kind: "number",
      hint: "Áp dụng cho cả LLM và Local MT.",
      disabledWhen: (v) => !v.auto_cleanup_han,
    },
  ...
  return (
    <SectionForm
      ...
      renderExtraActions={(draft, setDraft) => (
        <>
          <Badge tone={meta.prompt_template_overridden ? "gold" : "indigo"}>
            Prompt chương: {meta.prompt_template_overridden ? "ghi đè riêng" : "dùng chung"}
          </Badge>
          {meta.prompt_template_overridden ? (
            <Button
              size="sm"
              variant="ghost"
              title='Xóa nội dung ô "Prompt dịch chương" rồi bấm Lưu để dùng lại prompt Dịch chung — ô sẽ trống cho tới khi lưu'
              onClick={() => setDraft((current) => ({ ...current, prompt_template: "" }))}
            >
              Bỏ ghi đè
            </Button>
          ) : null}
          <Badge tone={meta.title_prompt_template_overridden ? "gold" : "indigo"}>
            Prompt tiêu đề: {meta.title_prompt_template_overridden ? "ghi đè riêng" : "dùng chung"}
          </Badge>
          {meta.title_prompt_template_overridden ? (
            <Button
              size="sm"
              variant="ghost"
              title='Xóa nội dung ô "Prompt dịch tiêu đề" rồi bấm Lưu để dùng lại prompt Dịch chung — ô sẽ trống cho tới khi lưu'
              onClick={() => setDraft((current) => ({ ...current, title_prompt_template: "" }))}
            >
              Bỏ ghi đè
            </Button>
          ) : null}
