# Spec: chapter-pagination

## Purpose

The crawler stitches multi-page chapters (sites that split a chapter into
several sub-pages with a "next page" link or a URL pattern) into a single
chapter text before handing it to the translator.
...
The crawler MUST stop paginating as soon as the trimmed text of the
newly fetched page is identical to the trimmed text of any page already
collected for the current chapter. This prevents infinite loops when a
"next" link is misconfigured (e.g. points to the same page).
...
The crawler MUST detect a repeated title at the start of a sub-page
(matching the first non-blank line of page 1) and strip it before
concatenation, so the resulting text does not contain the chapter title
twice.
