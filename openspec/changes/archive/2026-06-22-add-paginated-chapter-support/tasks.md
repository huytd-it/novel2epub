...
- [x] 3.1 Update `HttpCrawler.fetch_chapter` to delegate to
...
- [x] 3.2 Update `Crawl4AICrawler.fetch_chapter` with the same
      delegation, using `_arun` and `_markdown` + `_clean` as the
      extract closure
- [x] 3.3 Update `FirecrawlCrawler.fetch_chapter` with the same
      delegation, using `_scrape`
- [x] 3.4 Verify the existing `Crawler` protocol and the
      `pipeline.fetch_chapter` call site are unchanged

## 4. Ship the shuhaige preset

- [x] 4.1 Add a `shuhaige:` entry to `novel2epub/sources.yaml` with
      `engine: http`, `content_selector: "#content"`, and
      `next_page_selector: "a#pager_next"`
- [x] 4.2 Run `load_presets()` and confirm the new preset round-trips
      through YAML load and save without losing fields

## 5. Tests

- [x] 5.1 Add `tests/test_chapter_pagination.py` with fixtures that
      simulate a three-page chapter using `next_page_selector`
- [x] 5.2 Add a test for the URL-pattern fallback that monkey-patches
      `_get_soup` to return a known sequence of HTML responses
- [x] 5.3 Add a test that asserts duplicate-content detection stops
      the loop after the first duplicate
- [x] 5.4 Add a test that asserts `max_pages_per_chapter=3` caps the
      loop on a five-page fixture
- [x] 5.5 Add a test that asserts the repeated title on the first
      line of a sub-page is stripped
- [x] 5.6 Add a test that asserts `CrawlConfig` raises `ValueError` on
      a `next_page_url_pattern` with zero or two capturing groups
- [x] 5.7 Run `pytest tests/ -v` and confirm the full suite still
      passes

## 6. Documentation and smoke test

- [x] 6.1 Update `AGENTS.md` (or a new `docs/pagination.md`) with a
      short note explaining the three new fields and the shuhaige
      preset, plus a link to the shuhaige.net test URL the user
      originally reported
- [x] 6.2 Run a manual smoke test against
