...
- `idioms.py` — pure logic for the GLOBAL idiom/thành-ngữ dictionary (shared across ALL ebooks, unlike per-ebook glossary). Stored in the `idioms` SQLite table (schema v4, no `ebook_slug`) via `Storage.read/write/upsert/delete_idiom*` + `seed_idioms_if_empty`. Each idiom = `zh` (source) · `natural` (target, bản đẹp) · `literals` (bản máy MT hay ra, `|`-separated) · `protect` flag. Two application paths, one mode per idiom to avoid double-fail: LLM gets a soft reference block via the `{idioms}` prompt placeholder (filtered to the chunk, `filter_for_text`/`format_llm_block`); MT 57M uses `literal→natural` post-normalization (default, `normalize_literals`) or, for `@protect` idioms, `zh→placeholder` pre-MT then restore (`protect_source`/`apply_mt_post`, placeholder `〖N〗`). Toggle `translate.use_idioms` (default on). `_apply_glossary` applies longest-source-first (names never clobbered by shorter substrings). Web page `/idioms` (`app/routes/idioms.py` + `idioms.html`, global — not per-slug, slug sentinel `__global__` only builds the Storage/conn) reuses the glossary CRUD pattern; seeds 6 sample idioms from `test.md` when empty.
